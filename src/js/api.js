...
const VALID_PROTOCOLS = ['https:', 'mailto:', 'tel:'];
...
export async function fetchProfileConfig() {
    try {
        ...
        if (!response.ok) throw new Error('Profile config fetch failed');
...
export async function fetchLinksConfig() {
    try {
        const response = await fetch('/config/links.json');
        if (!response.ok) throw new Error('Links config fetch failed');
        return await response.json();
    } catch (error) {
        console.error('Error fetching links config:', error);
        return null;
    }
}
